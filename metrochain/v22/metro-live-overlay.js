...
window.__MC22_NETWORK_OVERLAY__='v22.15-clean-live';
...
function killLegacyCanvas(){const root=document.getElementById('liveMetroMap');if(!root)return;root.querySelectorAll('canvas.metro-train-canvas').forEach(c=>{c.style.display='none';c.style.opacity='0';c.style.pointerEvents='none';c.setAttribute('aria-hidden','true')})}
...
#liveMetroMap .mc22-vehicle{width:24px;height:24px;border-radius:999px;display:grid;place-items:center;background:#0d1b2a;border:3px solid var(--line);box-shadow:0 2px 8px rgba(0,0,0,.32);color:#fff;font:900 8px/1 system-ui;letter-spacing:-.03em;transition:opacity .2s ease}
#liveMetroMap .mc22-vehicle span{display:block;min-width:13px;text-align:center;color:#fff!important}
#liveMetroMap .mc22-vehicle.projected{opacity:.70;border-style:dashed}
...
function upsert(v,l,m){const id=m+':'+String(v.vehicle_id),lat=Number(v.latitude),lon=Number(v.longitude);let e=markers.get(id);const icon=()=>L.divIcon({className:'mc22-vehicle-host',html:markerHtml(v,l),iconSize:[26,26],iconAnchor:[13,13]});if(!e){const mk=L.marker([lat,lon],{icon:icon(),keyboard:false,zIndexOffset:1200});mk.bindTooltip(title(v,m),{direction:'top',offset:[0,-10]});mk.addTo(layer);e={mk};markers.set(id,e)}else{e.mk.setLatLng([lat,lon]);e.mk.setIcon(icon());if(e.mk.getTooltip())e.mk.setTooltipContent(title(v,m))}e.seen=true}
...
function start(tries=0){if(tries>220)return;installStyle();if(!map()||typeof L==='undefined'){setTimeout(()=>start(tries+1),75);return}ensureLayer();refresh();timer=setInterval(refresh,4000);document.addEventListener('click',e=>{if(e.target?.closest?.('.mc22-mode-btn'))setTimeout(refresh,120)});const root=document.getElementById('liveMetroMap');if(root)new MutationObserver(killLegacyCanvas).observe(root,{childList:true,subtree:true})}
start();
})();