(()=>{
  ...
  function enhanceJourney(){
    ...
    const fields=[...box.querySelectorAll('.place-field')];fields.forEach((f,i)=>f.classList.add(i===0?'mc21-field-from':'mc21-field-to'));
  ...
  function severity(text=''){
    ...
    if(/perturb|incident|travaux|ralenti|retard|dense|indispon|dégrad/.test(s))return 'orange';
    return 'green';
  }
  ...
  function trafficSummary(){
    ...
    const meaningful=candidates.find(t=>t.length<=190&&!/^(incidents?|chargement du trafic|actualiser|↻)$/i.test(t)&&/(\d+\s+incidents?|trafic normal|aucune perturbation|trafic perturb|travaux|interromp|ralenti|retard|incident|suspend|fermet)/i.test(t));
    const raw=meaningful||cleanText(strip.innerText||strip.textContent);
    return /chargement du trafic|incidents\s*[▾▼]|actualiser|↻/i.test(raw)&&!/(\d+\s+incidents?|trafic normal|aucune perturbation|perturb|travaux|interromp|ralenti|retard|suspend|fermet)/i.test(raw)?'Chargement du trafic en cours…':raw;
  }
  function collectIncidentTexts(){
    const live=d.getElementById('liveMap');if(!live)return[];
    const nodes=new Set([...live.querySelectorAll('[class*="incident"],[class*="disruption"],[class*="alert"],[class*="traffic"]'),...d.querySelectorAll('[class*="incident"],[class*="disruption"],[class*="alert"]')]);
    let out=[];
    ...
    const filtered=[];for(const t of out){if(filtered.some(x=>x.includes(t)||t.includes(x)))continue;filtered.push(t)}
  ...
  function makeIncidentPanel(){
    ...
    rail.insertBefore(p,rail.firstChild);refreshIncidents();
  }
  function refreshIncidents(){
    const panel=d.querySelector('.mc21-incidents');if(!panel)return;
    const raw=trafficSummary(),sev=severity(raw),count=raw.match(/(\d+)\s+incidents?/i)?.[1];
    ...
    if(title)title.textContent=count?`${count} incident${Number(count)>1?'s':''} en cours`:sev==='green'?'Trafic global normal':sev==='orange'?'Perturbations en cours':sev==='red'?'Incident majeur en cours':'État trafic en cours de chargement';
    ...
    const list=panel.querySelector('.mc21-incident-list');if(list){
      ...
      if(!items.length){list.innerHTML=sev==='loading'?'<div class="mc21-empty">Les détails des incidents apparaîtront ici dès que le flux trafic de MetroChain sera chargé.</div>':'<div class="mc21-empty">Aucun détail supplémentaire n’est exposé dans le DOM pour le moment. Le nombre et le statut ci-dessus restent synchronisés avec le flux MetroChain.</div>'}
  ...
  function pageLabel(main){const id=main?.id||'';return({liveMap:'Carte live',home:'Jeux',crosswordsScreen:'Mots croisés',progressScreen:'Progression',communityScreen:'Communauté',leaderboardScreen:'Classements',game:'Jeu',special:'Mode spécial'})[id]||'MetroChain'}
  function wireTransitions(){
    d.addEventListener('click',e=>{const b=e.target.closest?.('.nav-btn');if(!b||b.classList.contains('active'))return;playTransition(cleanText(b.textContent)||'Prochain arrêt')},true);
    let last=visibleMain()?.id||'';setInterval(()=>{const m=visibleMain(),id=m?.id||'';if(id&&id!==last){if(Date.now()>transitionUntil)playTransition(pageLabel(m));last=id}},220);
  }

  addAmbient();addTransition();enhanceJourney();makeIncidentPanel();wireTransitions();
  setInterval(refreshIncidents,900);
  setInterval(()=>{enhanceJourney();makeIncidentPanel()},3000);
})();